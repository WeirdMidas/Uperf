...
- Use PL1 (slowpowerlimit), PL2 (fastpowerlimit), TAU (fastpowercapacity) and TOU (fastlimitrecoveryscale) to respect the Android workload and the device's thermal limitations. On desktops, TAU can last from minutes to over 20 seconds, but this is because they have active cooling, whereas Android devices have passive cooling. Based on this, adapt PL1, PL2, TAU and TOU to be more consistent with the device's power consumption capacity through real-world data (from Geekbench) and also the SOC's focus. Performance chips have higher PL1 and PL2 values, while efficiency chips have lower PL1 and PL2 values ​​to operate within their physical capabilities without significant penalties in energy consumption and thermal heating. Based on respecting the chip's design and limitations, this approach will allow the energy model to be used at its full efficiency.
...
- Improve EAS decision-making by formulating a migration methodology that leverages the maximum potential of each specific processor's big.LITTLE architecture. This is achieved through **limitEfficiency** combined with a migration threshold that respects the throughput limits of the LITTLE cluster plus migration cost based on the relative capacity of the clusters. This allows Uperf to make significantly better decisions about task placement on its own, filtering out light and medium tasks from those that truly require power. This enables MAXIMIZING the performance-per-watt of big.LITTLE architectures.
