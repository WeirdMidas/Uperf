...
- Be more respectful of the scheduling methods of different architectures, such as the Android environment. Such as DynamlQ (SOCs with clusters in a single cluster along with PRIME cores) and Big.LITTLE (SOCs that have two or three clusters in a single cluster, such as those with prime cores). Allow Uperf to adapt to the scheduling methods of these SOCs and become more efficient per watt, avoiding wasted energy.
